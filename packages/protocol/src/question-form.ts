...
/**
 * The drafted answers that belong under question `qIndex`. A draft names its
 * question; one that names none came from a daemon that drafted a single set
 * for the whole card, so it is only safe where one question takes typed text.
 */
export function draftsForQuestion<T extends { questionIndex?: number }>(
  drafts: readonly T[],
  qIndex: number,
  questions: readonly QuestionFormQuestion[],
): T[] {
  const question = questions[qIndex];
  if (!question || !questionShowsTextInput(question)) {
    return [];
  }
  const soleTextQuestion = questions.filter(questionShowsTextInput).length === 1;
  return drafts.filter((draft) =>
    draft.questionIndex === undefined ? soleTextQuestion : draft.questionIndex === qIndex,
  );
}
