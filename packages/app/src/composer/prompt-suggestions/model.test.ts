...
const BASE = {
  ...
  lastActivityAt: new Date("2026-09-18T09:59:30.000Z"),
  hasText: false,
...
describe("resolvePromptSuggestionView", () => {
  ...
  it("drops a suggestion older than the agent's latest activity", () => {
    expect(
      resolvePromptSuggestionView({
        ...BASE,
        lastActivityAt: new Date("2026-09-18T10:00:30.000Z"),
      }).ghost,
    ).toBeNull();
  });

  it("keeps a suggestion when the agent has not moved since it was generated", () => {
    expect(resolvePromptSuggestionView({ ...BASE, lastActivityAt: undefined }).ghost).toEqual(
      SUGGESTIONS[0],
    );
  });
